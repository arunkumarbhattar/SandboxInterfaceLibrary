...
namespace wasm2c_detail {
  ...
  {
    //static_assert(std::is_void<T>::value, "Missing specialization");
    using type = void;
  ...
  struct convert_type_to_wasm_type<
    T,
    std::enable_if<(std::is_integral<T>::value || std::is_enum<T>::value)&&sizeof(T) <=
                     sizeof(uint32_t)>>
  {
    using type = uint32_t;
    static constexpr wasm_rt_type_t wasm2c_type = WASM_RT_I32;
  ...
  struct convert_type_to_wasm_type<
    ...
    std::enable_if_t<(std::is_integral<T>::value ||
                      std::is_enum<T>::value)&&sizeof(uint32_t) < sizeof(T) &&
  ...
  struct convert_type_to_wasm_type<
    T,
    std::enable_if_t<std::is_pointer<T>::value || std::is_class<T>::value>>
  {
  ...
  namespace change_class_arg_types_detail {
    ...
    {
      using type =
        T_Ret(std::conditional_t<std::is_class<T_Args>::value, T_ArgNew, T_Args>...);
    };
  }
...
__attribute__((weak))
  ...
  {
    if constexpr (std::is_class<T_FormalRet>::value) {
  ...
  ) const
  {
    ...
    constexpr bool promoted = std::is_class<T_Ret>::value;
    ...
    if constexpr (promoted) {
      ...
      ret_count = std::is_void<T_Ret>::value? 0 : 1;
    }
...
protected:
  ...
  void* impl_lookup_symbol(const char* func_name)
  {
    constexpr bool fail = std::is_same<T, void>::value;
    static_assert(
      !fail,
...
public:
  ...
  auto impl_invoke_with_func_ptr(T_Converted* func_ptr, T_Args&&... params)
  {
    ...
    if constexpr (std::is_class<T_Ret>::value) {
    ...
    constexpr size_t alloc_length = [&] {
      if constexpr (sizeof...(params) > 0) {
        return ((std::is_class<T_Args>::value ? 1 : 0) + ...);
      } else {
        return 0;
      }
    }();
    ...
    auto serialize_class_arg =
      [&](auto arg) -> std::conditional_t<std::is_class<decltype(arg)>::value,
                                          T_PointerType,
                                          decltype(arg)> {
      using T_Arg = decltype(arg);
      if constexpr (std::is_class<T_Arg>::value) {
    ...
    using T_NoVoidRet =
      std::conditional_t<std::is_void<T_Ret>::value, uint32_t, T_Ret>;
    T_NoVoidRet ret;
